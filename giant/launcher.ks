...
function Launch {
  ...
  On time:seconds {
    ...
    Set control:pitch to CONTROL_FACTOR * pid_pitch:update(time:seconds, -vdot(ship:angularvel, ship:facing:starvector)). 
    Set control:yaw to CONTROL_FACTOR * pid_yaw:update(time:seconds, vdot(ship:angularvel, ship:facing:upvector)). 
  ...
  When verticalspeed > 100 then {
    ...
    Set control:pitch to CONTROL_FACTOR * 0.05.
