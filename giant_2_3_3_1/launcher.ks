...
Local function project_away_from {
  Parameter vec.
  Parameter ortho.

  Set ortho to ortho:normalized.

  Return vec - vdot(vec, ortho) * ortho.
}
...
function Launch {
  ...
  Local pid_pitch is pidloop(3.2, 32.0, 0.0, -1, 1).
  Local pid_yaw is pidloop(3.2, 12.8, 0.0, -1, 1).
  ...
  Local control_factor to 2.0.
  On stage:number {
    Set control_factor to 1.0.
  }
  ...
  When verticalspeed > 100 then {
    ...
    On time:seconds {
      ...
      Set control:pitch to control_factor * pid_pitch:update(time:seconds, -vdot(vcrs(ship:srfprograde:forevector, ship:facing:forevector), ship:facing:starvector)).
      If vang(ship:facing:forevector, ship:up:vector) < TURN_ANGLE {
        ...
        On time:seconds {
          ...
          Set control:pitch to control_factor * 4.0 * dd_pitch:update(time:seconds, pi_pitch:changerate - desired_changerate_pitch).
          ...
          If vang(ship:facing:forevector, project_away_from(ship:srfprograde:forevector, ship:facing:starvector)) > 0.5 {
            Return true.
          }
