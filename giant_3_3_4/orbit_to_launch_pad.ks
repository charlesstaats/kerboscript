...
Parameter profile is "0:/giant_3_3_4/descent_profile.csv".
Runpath("0:/giant_3_3_4/rcs_lock_to_target").
...
Local steering_airbrakes is ship:partsdubbed("steering_brake").
Local airbrakes is ship:partsdubbedpattern("airbrake").
Set airbrakes to Enum:reject(airbrakes, { Parameter part. Return part:tag = "steering_brake". }).
...
Local function unlock_steering_airbrakes {
  ...
  For airbrake in steering_airbrakes {
    Local power is clip(steering_power, 0, 1).
    Local aero_surface_module to airbrake:getmodule("ModuleAeroSurface").
    Aero_surface_module:setfield("authority limiter", power * 100).
    Aero_surface_module:doaction("retract", true).
  }
}

Local function lock_steering_airbrakes {
  Parameter braking_power.
  
  For airbrake in steering_airbrakes {
    Local power is clip(braking_power, 0, 1).
    Local aero_surface_module to airbrake:getmodule("ModuleAeroSurface").
    Aero_surface_module:setfield("authority limiter", power * 100).
    Aero_surface_module:doaction("extend", true).
  }
}
...
If LOG_TRAJECTORY {
  Local log_file is "0:/giant_3_3_4/descent_log.csv".
...
Local function lock_altitude_retrograde {
  ...
  Local pid_alt_multiplier is 1e-4.
...
Local function lock_anti_target {
  ...
  On time:seconds {
    ...
    Set target_direction to (target_direction + 0.55 * (target_direction - ship:up:forevector)):normalized.
...
When altitude < 70000 then {
  ...
  On time:seconds {
    ...
    If alt:radar < 20000 {
      Set_airbrakes(1.0).
      Lock_steering_airbrakes(1.0).
      Return false.
    }
    Set_airbrakes(0.75 + pid_ergy:update(time:seconds, expected_ergy_at(phase_angle) - ergy())).
...
Local function distortion_vector {
  ...
  Return 40 * upvec + 0.16 * targetvec.
}

When throttle > 0.05 then {
  RCS on.
  Brakes off.
  Lock_steering_airbrakes(0.0).
