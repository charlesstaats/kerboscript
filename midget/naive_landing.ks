...
//For line in open("0:/jool/descent_profile.csv"):readall {
//  If not header_seen {
...
Local function lock_altitude_retrograde {
  ...
  Local pid_pitch is pidloop(1.0, 0.0, 60.0, -1, 1).
  Local pid_yaw is pidloop(2.0, 0.0, 120.0, -1, 1).
  ...
  When alt:radar < 20000 then {
    ...
    When alt:radar < 1000 then {
      ...
      When (alt:radar < 150 and (target:position - vdot(target:position, ship:up:forevector) * ship:up:forevector):mag < 10)
           or (alt:radar < 50 and target:position:mag > 500)  // The "abort" case.
           then
      {
