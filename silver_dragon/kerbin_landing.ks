...
Local function set_airbrakes {
  ...
  For airbrake in airbrakes {
    ...
    Local deployed is airbrake:getmodule("ModuleAeroSurface"):getfield("deploy").
...
When alt:radar < 1000 then {
  ...
  Transfer_order:activate().  // Give the engines as much leverage as possible.
  Local pid_thrust is pidloop(0.125, 0, 0.5, 0, 1).
  Set pid_thrust:setpoint to 10.
  Local bounds is ship:bounds.
  Lock throttle to pid_thrust:update(time:seconds, bounds:bottomaltradar).
  When target:position:mag < 120 then { Set pid_thrust:setpoint to -0.5. }
}
