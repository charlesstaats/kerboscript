...
If LOG_TRAJECTORY {
  ...
  When time:seconds >= next_log_time then {
    Set next_log_time to next_log_time + (choose 10 if altitude > 15000 and currently_logging else 1).
...
Local function lock_altitude_retrograde {
  ...
  Local pid_pitch is pidloop(0.25, 0.0, 10.0, -1, 1).
  Local pid_yaw is pidloop(0.25, 0.0, 10.0, -1, 1).
...
Local function lock_anti_target {
  ...
  On time:seconds {
    ...
    Set control:pitch to control_factor * pid_pitch:update(time:seconds,
      -vdot(target_direction, ship:facing:topvector)).
    Set control:yaw to control_factor * pid_yaw:update(time:seconds,
      -vdot(target_direction, ship:facing:starvector)).
    Return true.
  }
}
...
When altitude < 70000 then {
  ...
  When alt:radar < 20000 then {
    ...
    When alt:radar < 1000 then {
      ...
      Local pid_thrust is pidloop(0.12, 0, 0.5, 0, 1).
...
When throttle > 0.05 then {
  ...
  Local pid_pitch is pidloop(2.0, 0.4, 8.0, -1, 1).
  Local pid_yaw is pidloop(4.0, 0.4, 8.0, -1, 1).
