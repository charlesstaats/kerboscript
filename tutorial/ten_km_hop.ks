...
local surface_gravity to body:mu / body:radius^2.
local function potential_energy_at_height { 
  ...
  return ship:mass * surface_gravity * height.  // mgh
}
...
local function update_throttle_going_down {
  ...
  set throttle_var to max(MIN_THROTTLE,
      weight / ship:availablethrust +
      0.1 * throttle_pid:update(time:seconds, ship:verticalspeed - desired_verticalspeed)).
}
...
local function update_steering {
  ...
  if ship:Q > 0.75 * throttle_var and ship:facing:vector * ship:velocity:surface < 0 {
    set look_at_horiz to -look_at_horiz.
  }
